...
console = Console()
console.clear()
ctypes.windll.kernel32.SetConsoleTitleW('Mizogg Corp.(guarda.py)')
mizogg= '''[red]
              ___            ___  
             (o o)          (o o) 
            (  V  ) MIZOGG (  V  )
...
def check_balance(addr, dec, HEX, count, count1):
    try:
        url = 'https://btcbook.guarda.co/api/v1/address/'+ addr
        ...
        if response.status == 200:
            ...
            console.print(Panel(str(running_print_main) , title = "[white]\n mizogg.co.uk guarda.py [/]" , subtitle = "[green_yellow blink] Good Luck Happy Hunting [/]" , style = "green") , style = style , justify = "full")
            if int(txs) > 0:
                length = len(bin(dec))
                length -=2
